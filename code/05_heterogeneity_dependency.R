...
output <- do.call(rbind, lapply(outcomes_principais, function(feature) {
  bind_rows(
    process_plot_data(df = subset(df,is_publica == 1), feature, type = "time_effect") %>% mutate(tipo = 'Public'),
    process_plot_data(df = subset(df,is_privada == 1), feature, type = "time_effect") %>% mutate(tipo = 'Private'),
  )
}))
...
# Salvando o gráfico
ggsave(filename  = './results/dependencia.jpg',
