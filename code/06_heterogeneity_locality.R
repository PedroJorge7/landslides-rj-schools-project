...
output <- do.call(rbind, lapply(outcomes_principais, function(feature) {
  bind_rows(
    process_plot_data(df = subset(df,is_urbana == 1), feature, type = "time_effect") %>% mutate(tipo = 'Urban'),
    process_plot_data(df = subset(df,is_rural == 1), feature, type = "time_effect") %>% mutate(tipo = 'Rural'),
  )
}))
...
# Salvando o gráfico
ggsave(filename  = './results/localizacao.jpg',
